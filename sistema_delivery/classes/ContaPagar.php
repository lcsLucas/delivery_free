...
class ContaPagar implements Interfaceclasses
{
    ...
    public function __construct($num_parcela=0, $data_vencimento="", $data_pago="", $valor=0, $valor_pago=0, $obs="", $flag_entrada="", $idEntrada = null)
    {
        ...
        $this->data_vencimento = empty($data_vencimento) ? date("Y-m-d") : $data_vencimento;
        $this->data_pago = empty($data_pago) ? date("Y-m-d") : $data_pago;
        $this->valor = $valor;
    ...
    /**
     * @param false|string $data_vencimento
     */
    ...
    public function alterar()
    {
        ...
        $resp = $crud->AlteraCondicoes("contas_pagar",
    ...
    {
        $crud = new Crud();
        $resp = $crud->ExcluirCondicoes("contas_pagar", "con_id = " . $this->id . " AND emp_id = " . $this->idEmpresa);
    ...
    public function quantidadeRegistros2($periodo1, $periodo2)
    {
        ...
        if(!empty($periodo1) && !empty($periodo2)){
			...
			$resp = $crud->ConsultaGenerica("SELECT count(*) total FROM contas_pagar WHERE emp_id = ? AND con_dtVencimento BETWEEN ? AND ?", array($this->idEmpresa, $periodo1, $periodo2));
        } else {
            $resp = $crud->ConsultaGenerica("SELECT count(*) total FROM contas_pagar WHERE emp_id = ? ", array($this->idEmpresa));
        }
    ...
    public function listarPaginacao2($periodo1, $periodo2, $inicio, $fim)
    {
		...
		if(!empty($periodo1) && !empty($periodo2)){
			...
			$res = $crud->Consulta("contas_pagar WHERE emp_id = ? AND con_dtVencimento BETWEEN ? AND ? order by con_id desc LIMIT ?, ?;", array($this->idEmpresa, $periodo1, $periodo2, $inicio, $fim));
        }
        else {
            $res = $crud->Consulta("contas_pagar WHERE emp_id = ? order by con_id desc LIMIT ?, ?;", array($this->idEmpresa, $inicio, $fim));
        }
    ...
    public function listarRelatorio($periodo1='', $periodo2='') {
		...
		if(!empty($periodo1) && !empty($periodo2)){
			...
			$res = $crud->Consulta("contas_pagar WHERE emp_id = ? AND con_dtVencimento BETWEEN ? AND ? order by con_id desc", array($this->idEmpresa, $periodo1, $periodo2));
		}
		else {
			$res = $crud->Consulta("contas_pagar WHERE emp_id = ? order by con_id desc", array($this->idEmpresa));
		}
    ...
    {
        $crud = new Crud();
        $resp = $crud->Consulta("contas_pagar WHERE con_id = ? AND emp_id = ? LIMIT 1", array($this->id, $this->idEmpresa));
        ...
        if (!empty($resp)) {
            ...
            $this->flag_entrada = $resp[0]["con_entrada"];
