<?php 
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') { //requisição post
        ...
        } else if (filter_has_var(INPUT_POST, "addPermissao")) {
            $id_menu = filter_input(INPUT_POST, 'menu', FILTER_VALIDATE_INT);
            $id_usu = filter_input(INPUT_POST, 'idus', FILTER_VALIDATE_INT);

            $resp = $submenu->relacionarSubMenuUsuario($id_menu, $id_usu);
            if ($resp == TRUE) {
                $sucessorelacionar = TRUE;
            } else {
                $errorelacionar = TRUE;
            }
        } else if (filter_has_var(INPUT_POST, "remPermissao")) {
            $id_menu = filter_input(INPUT_POST, 'menu', FILTER_VALIDATE_INT);
            $id_usu = filter_input(INPUT_POST, 'idus', FILTER_VALIDATE_INT);
            $resp = $submenu->desrelacionarSubMenuUsuario($id_menu, $id_usu);
            if ($resp == TRUE) {
...
$lista = $submenu->listar();
$listatipousuarios = $tipousuarios->listar();
$listmenu = $menu->listar();
...
<div class="card">
    <div class="card-header <?= empty($carregado) ? "bg-primary" : "bg-danger" ?> text-white">
        <h5 class="titulo-card">
            <?= empty($carregado) ? "Cadastrar Novo SubMenu" : "Alterar o SubMenu <q>$descricao</q>" ?>
        </h5>
    </div>
    <div class="card-body">
                        ...
                        <tr>
                            ...
                            <td class="text-center">
                                <form method="post" action="<?= $_SERVER["PHP_SELF"]; ?>">
                                    ...
                                    if ($rs['status']) {
                                        ?>
                                        <button type="submit" title="Clique para Desativar o SubMenu" class="btn btn-link" name="alterar-status" ><i class="fa fa-check-square-o fa-2x text-success" aria-hidden="true"></i></button>
                                        <?php
                                    } else {
                                        ?>
                                        <button type="submit" title="Clique para Ativar o SubMenu" class="btn btn-link" name="alterar-status" ><i class="fa fa-square-o fa-2x text-danger" aria-hidden="true"></i></button>
                                    <?php } ?>
                                </form>
                            </td>
                            <td class="text-center">
                                <form method="post" action="<?= $_SERVER["PHP_SELF"]; ?>">
                                    <input type="hidden" name="acao-codigo" value="<?= $rs['idsubmenu'] ?>" />
                                    <button type="submit" class="btn btn-info btn-acao" title="Editar SubMenu" name="editar">
                                        <i class="fa fa-pencil" aria-hidden="true"></i>
                                    </button>
                                    <button type="submit" class="btn btn-danger btn-acao excluir" name="deletar" title="Excluir SubMenu">
...
<div class="card">
    <div class="card-header bg-primary text-white">
        <h5 class="titulo-card">
            Delegar Permissões de SubMenus
        </h5>
    </div>
    <div class="body">
        <div class="table-responsive">
            <table class="table table-striped table-hover dataTable">
                <thead>
                <tr>
                    <th class="col-auto">Menus / Tipos de usuários</th>
                    <?php
                    if (!empty($listatipousuarios)) {
                        foreach ($listatipousuarios as $rsa) {
                            ?>
                            <th class="text-center col-auto not-ordering"><?php echo utf8_encode($rsa['nome_tipo']) ?></th>
                            <?php
                        }
                    }
                    ?>
                        ...
                        <tr>
                            ...
                            if (isset($listatipousuarios)) {
                                foreach ($listatipousuarios as $rsa) {
                                    ...
                                    <td class='text-center'>
                                        <form method="post" action="<?= $_SERVER["PHP_SELF"]; ?>">
                                            <input type="hidden" name="menu" value="<?= $rs["idsubmenu"] ?>">
                                            <input type="hidden" name="idus" value="<?= $rsa["idusuariotipo"] ?>">
                                            <?php
                                            if ($submenu->verificaRelacao($rs["idsubmenu"], $rsa["idusuariotipo"])) {
                                                ?>
                                                <button type="submit" title="<?php echo $rsa['nome_tipo']; ?>" class="btn btn-link" name="remPermissao" ><i class="fa fa-check-square-o fa-2x text-success" aria-hidden="true"></i></button>
                                                <?php
                                            } else {
                                                ?>
                                                <button type="submit" title="<?php echo $rsa['nome_tipo']; ?>" class="btn btn-link" name="addPermissao" ><i class="fa fa-square-o fa-2x text-danger" aria-hidden="true"></i></button>
                                                <?php
                                            }
                                            ?>
                                        </form>
