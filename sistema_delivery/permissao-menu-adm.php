...
<h2 class="titulo-pagina">Gerenciar Permissões dos Menus das Empresas</h2>
                        ...
                        <table id="dataTable"
                            ...
                            if (!empty($lista_menu)) {
                                foreach ($lista_menu as $result_menu) {
                                    ...
                                    <tr>
                                        <td>
                                            <?= utf8_encode($result_menu["descricao_menu"]) ?>
                                        </td>
                                        <td class="text-center">
                                            <?php
                                            if ($menu_emp->verificaRelacaoEmpresa($result_menu["idmenu"], $lista_usuarios["usu_adm"], $lista_usuarios["emp_id"]) == false) {
                                                ?>
                                                <a role="button"
                                                   title="Clique para remover a permissão desse usuário para esse menu"
                                                   class="btn btn-link"
                                                   href="?menu=<?= $result_menu["idmenu"]; ?>&adpe=1&idus=<?= $lista_usuarios["usu_adm"]; ?>&idemp=<?= $lista_usuarios["emp_id"]; ?>"><i
                                                ...
                                                <a role="button"
                                                   title="Clique para remover a permissão desse usuário para esse menu"
                                                   class="btn btn-link"
                                                   href="?menu=<?php echo $result_menu["idmenu"]; ?>&rmpe=1&idus=<?php echo $lista_usuarios["usu_adm"]; ?>&idemp=<?= $lista_usuarios["emp_id"]; ?>"><i
