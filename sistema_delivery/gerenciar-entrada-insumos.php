<?php
            ...
            if (empty($data) || empty($id_pag)) {
                ...
                if (filter_has_var(INPUT_POST, "editar")) {
                    ...
                    if ($resp) {
                        ...
                        $id = $id_nota = $data = $id_pag = $observacao = "";
                        $frete = $desconto = $outros = 0.00;
                        unset($_SESSION["_insumosentrada"]);
                    } else {
                        $erroalterar = TRUE;
                        $carregado = filter_has_var(INPUT_POST, "editar") ? true : false;
                    }
                } else {
                    $resp = $entrada->inserir_insumos();
                    if ($resp) {
                        $sucessoinserir = TRUE;
                        $id = $id_nota = $data = $id_pag = $observacao = "";
                        $frete = $desconto = $outros = 0.00;
    ...
    if (!empty($carregado)) {
        ...
        $lista_insumos = $entrada->recuperaInsumos();

        foreach ($lista_insumos as $i => $item) {

            $qtde = intval($item["qtdeinsumo"]);
            $preco = floatval($item["precoinsumo"]);

            $total += floatval($item["totalinsumo"]);

            $item["nomeinsumo"] = utf8_encode($item["nomeinsumo"]);
            $item["qtdeinsumo"] = number_format($item["qtdeinsumo"], 2, ",", ".");
            $item["precoinsumo"] = number_format($item["precoinsumo"], 2, ",", ".");
            $item["totalinsumo"] = number_format($item["totalinsumo"], 2, ",", ".");

            $lista_insumos[$i] = $item;
        }
    ...
    if (!empty($_SESSION["_insumosentrada"])) {
        ...
        foreach ($_SESSION["_insumosentrada"] as $i => $itens_session) {
            $item_compra->setId($i);
...
$num_rows = $entrada->quantidadeRegistros3($periodo1, $periodo2);
$lista = $entrada->listarPaginacao3($periodo1, $periodo2, $offset,$entries_per_page);
...
?>
    ...
    <h2 class="titulo-pagina">Entrada de Insumos</h2>
    ...
    <div class="card mb-3 <?= empty($carregado) ? "border-primary" : "border-danger" ?>">
        ...
        <div class="card-header <?= empty($carregado) ? "bg-primary" : "bg-danger" ?> text-white">
            <i class="fa fa-shopping-cart"></i> <?= empty($carregado) ? "Cadastrar Nova Entrada de Insumos" : "Alterar Informações da Entrada Nº " . $id ?>
        </div>

        <div class="card-body">

            <form id="form-entrada-insumos" class="needs-validation" action="<?= $_SERVER["PHP_SELF"] ?>" method="post" autocomplete="off" novalidate>
                        ...
                        <div class="row">
                            ...
                            <div class="col-12 col-xl-4">
                                ...
                                <div class="form-group">
                                    <label class="font-weight-bold" for="insumo">Insumo:</label>
                                    <select <?= !empty($carregado) ? "disabled" : "" ?> id="sel-insumo" name="insumo" class="form-control select2">
                                        ...
                                        if (!empty($todos_insumos)) {
                                            foreach($todos_insumos as $insumo) {
                                                ...
                                                <option data-medida="<?= $insumo["uni_sigla"] ?>" data- value="<?= $insumo["ins_id"] ?>"><?= utf8_encode($insumo["ins_nome"]) ?></option>
                            ...
                            <div class="col-12 col-xl-2 text-center">
                                ...
                                <div class="form-group">
                                    <a id="btn-add-insumo" href="" class="btn btn-outline-primary btn-block btn-lg <?= !empty($carregado) ? "disabled" : "" ?>">Adicionar</a>
                                </div>
                            ...
                            <div class="col-12 mt-5">
                                ...
                                <div class="table-responsive">
                                    ...
                                    <table id="lista-insumo" class="table" width="100%" cellspacing="0">
                                        <thead>
                                        <tr>
                                            <th>Insumo</th>
                                        ...
                                        if (!empty($lista_insumos)) {
                                            foreach ($lista_insumos as $item) {
                                            ...
                                            <tr>
                                                ...
                                                <td class="text-center">
                                                    <a data-id="<?= empty($carregado) ? $item["idinsumo"] : "" ?>" class="btn btn-danger btn-acao btn-excluir-insumo <?= !empty($carregado) ? "disabled" : "" ?>"
                                            ...
                                            <tr>
                                                ...
                                                <td colspan="6" class="text-muted text-center">Nenhum Insumo Adicionado</td>
      ...
      <div class="card mb-3 border-primary">
        <div class="card-header bg-primary text-white">
          Relatório de Entradas de Insumos <a target="_blank" href="relatorios/relatorio-entradas-insumos.php?filtro=<?= $filtro . $parametros ?>" class="float-right border-white btn btn-outline-secondary"><i class="fa text-white fa-print"></i></a>
        </div>
