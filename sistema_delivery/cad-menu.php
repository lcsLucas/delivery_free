...
if ($_SERVER['REQUEST_METHOD'] === 'POST') { //requisição post
    if (filter_has_var(INPUT_POST, 'btnEnviar')) {// enviada do formulario de cadastro/alteração
        ...
        $resp = $menu->relacionarMenuUsuario($id_menu, $id_usu);
        if ($resp == TRUE) {
        ...
        $id_menu = filter_input(INPUT_POST, 'menu', FILTER_VALIDATE_INT);
        $id_usu = filter_input(INPUT_POST, 'idus', FILTER_VALIDATE_INT);
        $resp = $menu->desrelacionarMenuUsuario($id_menu, $id_usu);
        if ($resp == TRUE) {
...
<h2 class="titulo-pagina">Gerenciar Cadastro de Menus</h2>
                                ...
                                if (isset($listatipousuarios)) {
                                    foreach ($listatipousuarios as $rsa) {
                                        ?>
                                            <td class='text-center'>
                                                <form method="post" action="<?= $_SERVER["PHP_SELF"]; ?>">
                                                    <input type="hidden" name="menu" value="<?= $rs["idmenu"] ?>">
                                                    <input type="hidden" name="idus" value="<?= $rsa["idusuariotipo"] ?>">
                                                <?php
                                                if ($menu->verificaRelacao($rs["idmenu"], $rsa["idusuariotipo"])) {
                                                    ?>
                                                    <button type="submit" title="<?php echo utf8_encode($rsa['nome_tipo']) ?>" class="btn btn-link" name="remPermissao" ><i class="fa fa-check-square-o fa-2x text-success" aria-hidden="true"></i></button>
                                                    <?php
                                                } else {
                                                    ?>
                                                    <button type="submit" title="<?php echo utf8_encode($rsa['nome_tipo']) ?>" class="btn btn-link" name="addPermissao" ><i class="fa fa-square-o fa-2x text-danger" aria-hidden="true"></i></button>
                                                    <?php
                                                }
                                                ?>
                                                </form>
